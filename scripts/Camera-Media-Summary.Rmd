...
# load in datasets 
trapdays <- read.csv(here(file = "data/camera-data/camera_days.csv")) # 
...
study_days <- trapdays %>% 
  ...
  group_by(date) %>% 
  summarise(n_cameras_deployed = n_distinct(site_id)) # calculate # of cameras funtional per day 
...
study_period_plot <- ggplot(functional_trap_days, aes(x = date)) + 
  geom_bar()
study_period_plot
#
deploy_date <- days %>%
...
deployment_data_wide <- days %>%
  filter(days_since_deployment != "???") %>% filter(days_since_deployment != "n/a") %>%
  select(site_id, date, trap_day) %>%
  arrange(date) %>%
  pivot_wider(names_from = date, values_from = trap_day)
# deployment_data <- colSums(!is.na(deployment_data_wide)) %>%
  # as.data.frame(deployment_data) %>% setNames(c("sites_deployed"))
# ggplot2(data = deployment_data, aes(x = ))
```
