...
days <- read.csv(file = "willoughby-ringtail-fieldwork/data/camera-data/camera_days.csv")
events <- read.csv(file = "willoughby-ringtail-fieldwork/data/camera-data/check_event.csv")
detection <- read.csv(file = "willoughby-ringtail-fieldwork/data/camera-data/detection_info.csv")
...
From STARTDATE, cameras were deployed at NUMBER sites for a total of NUMBER camera days. NUMBER (PERCENT) cameras are located at natural sites and NUMBER (PERCENT) are located at building sites. The cameras were functional for NUMBER days, PERCENT of the study period. Camera function was interrupted when a camera had a full SD card (NUMBER OF DAYS [PERCENT]), a dead battery (NUMBER OF DAYS [PERCENT]), or the camera being misplaced (NUMBER OF DAYS [PERCENT]).
