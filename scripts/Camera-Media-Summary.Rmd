...
Camera data is collected periodically in check events. Across all sites, camera data has been collected `r nrow(events)` times. Each check event covers an average of `r mean_event_duration` camera days, with an average of NUMBER (PERCENT) functional camera days.
...
study_period_plot <- ggplot(study_days) +
  geom_col(aes(x = date, y = n_cameras_deployed)) +
  labs(y = "Number of cameras deployed")
study_period_plot
