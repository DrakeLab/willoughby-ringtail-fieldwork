...
From `r start_date`, cameras were deployed at `r nrow(site_list)` sites over `r format(nrow(study_days), big.mark = ",")` days, for a total of `r format(nrow(camera_days_count), big.mark = ",")` camera days. `r nrow(count_natural)` (`r prop_natural`%) cameras are located at natural sites and `r nrow(count_building)` (`r prop_building`%) are located at building sites. The cameras were functional for `r format(nrow(site_days_functional), big.mark = ",")` camera days, `r prop_functional`% of the study period. Camera function was interrupted when a camera had a full SD card (`r format(nrow(site_days_sd_full), big.mark = ",")` camera days [`r prop_sd_full`%]), a dead battery (`r format(nrow(site_days_batteries_dead), big.mark = ",")` camera days [`r prop_batteries_dead`%]), or was dislodged or misplaced (`r format(nrow(site_days_dislodged), big.mark = ",")` camera days [`r prop_dislodged`%]).

Camera data is collected seasonally, on average every 3 months. Across all sites, camera data has been collected `r format(nrow(events), big.mark = ",")` times. Each check event covers an average of `r mean_event_duration` camera days, with an average of NUMBER (PERCENT) functional camera days.
...
Camera data is stored by check event. Of the `r nrow(events)` check events, `r backup` (`r prop_backup`%) are stored in two physical locations. `r upload_zooniverse$n_upload_zooniverse` (`r prop_upload`%) are uploaded to the Canyon Critters Zooniverse project.

Camera data is not always aligned to the correct time (X metric) and check events occured in two timezones (). Time adjustments have been confirmed for X, but need to be addressed for X trigger events. Of the incorrect times, X% have been corrected to UTC.   
...
Across all sites, cameras captured `r format(count_triggers_total, big.mark = ",")` trigger events, of which `r format(count_detections, big.mark = ",")` (`r prop_detections`%) were animal detections. Cameras deployed at natural sites recorded two photographs per trigger event. In total, natural sites captured `r format(count_triggers_natural, big.mark = ",")` trigger events, covering DURATION. Cameras deployed at building sites recorded videos in 20, 30, or 60-second durations. In total, building sites captured `r format(count_triggers_building, big.mark = ",")` trigger events, covering DURATION.
