...
#flatten_json function # SLOW VERSION - took launch 1 about 40 minutes 
...
## not functional rn 
flatten_json_optimized <- function(json_data) {
  # Convert to data.table for better performance
  json_dt <- as.data.table(json_data)
  
  # Select only the necessary columns
  json_dt <- json_dt[, .(subject_ids, user_name, classification_id, workflow_version, annotations)]
  
  # Flatten the JSON column
  flat_to_task <- json_dt[, .(task_index = seq_len(.N), annotations = annotations)] %>%
    .[, .(task_index, task =task, task_label = task_label)]
